...
static int bpf_token_get_info_by_fd(struct file *file,
				    ...
				    union bpf_attr __user *uattr)
{
	...
	if (err)
		return err;
	return token_get_info_by_fd(token, attr, uattr);
}
				  ...
				  union bpf_attr __user *uattr)
{
	...
	if (fd_file(f)->f_op == &bpf_prog_fops)
		...
		return bpf_token_get_info_by_fd(fd_file(f), fd_file(f)->private_data,
						attr, uattr);
	return -EINVAL;
}
